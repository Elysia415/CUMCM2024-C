...
import numpy as np
def greedysolve(muchan, pqall, descenddict, get_row_number, season):
    ...
    while len(pqall) > 0:
        ...
        for i in range(len(_ground[grid])):
            if maxsold[pldi] / muchan[(pldi, grid)] <= limitminarea[grid]:
                planted = True
                heapq.heappop(pqall)  # (1)(2)问不同点
                if not changed[(pldi, grid)]:
                    changed[(pldi, grid)] = 1
                    heapq.heappush(pqall, (-descenddict[(pldi, grid)], pldi, grid))
                    # 再给定一个限额
                    maxsold[pldi] += maxsold_2[pldi]
                break
...
while year < 2030:
    ...
    muchanlog = defaultdict(lambda: defaultdict(int))
    # 第一季
    ...
    profit1 = greedysolve(muchan1, pqall1, descenddict1, get_row_number1, 1)
    profit2 = greedysolve(muchan2, pqall2, descenddict2, get_row_number2, 2)
    year += 1
    ...
    for adict in data1_1[1:]:
        ...
        if adict['C0'] in list(range(17, 38)):
            adict['C2'] *= (100 + 5 + np.random.randn()) / 100
        elif adict['C0'] in list(range(39, 41)):
            adict['C2'] *= (100 - 1 - 4 * np.random.random()) / 100
        elif adict['C0'] == 41:
            adict['C2'] *= (100 - 5 - np.random.randn()) / 100
